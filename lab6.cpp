...
char getMove(char world[ROWS][COLS], int playerRow, int playerCol)
{
    ...
    if(world[playerRow][playerCol+1] != 'X') {
            indexDirections++;
            directions[indexDirections] ='r';
        }
    
    // print valid directions.
    // for i=0 to < indexDirections-1, i++
    // print elements of arrays.
    for(int i=0; i<=indexDirections-1; i++){
        cout<<directions[i]<<", ";
    }
    cout<<directions[indexDirections]<<endl;
    ...
    for (int i = 0; i <= indexDirections; ++i)
    {
        if(move==directions[i]){
            cout<<"It's Valid direction."<<endl;
            break;
        }
        else{
                while(move!=directions[i]){
                    if(i<indexDirections){
                        break;
                    }
                    else{
                        cout<<"Please enter valid direction: ";
                        cin>>move;
                        i=0;
                    }
                }   
                 ...
                 int &playerRow, int &playerCol)
{
    ...
    //      status = ONGOING 
    //      get extra point
    else if(world[playerRow][playerCol]=='T'){
        status = ONGOING;
        cout<<"You found a treasure! You gain extra point."<<endl;
    }
    ...
    //      status = WON
    else if(world[playerRow][playerCol]=='G'){
        status = WON;
    }
